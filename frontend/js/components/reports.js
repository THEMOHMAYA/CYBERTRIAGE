// Report Generator View (Export to PDF & Printable Formal Forensic Document)
...
export async function renderReports(container, activeCase, navigateTo) {
  container.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;">
      <div>
        <h2 style="font-size:1.5rem; font-weight:700; color:#FFFFFF;">Investigation Report Generator</h2>
        <p style="font-size:0.875rem; color:var(--text-secondary);">
          Compile comprehensive, evidence-backed forensic examination reports with cryptographic audit trails and export as official PDF.
        </p>
      </div>

      <div style="display:flex; gap:0.75rem;">
        <button class="btn btn-secondary" id="btn-print-report" style="display:none;">
    ...
    <div class="dfir-card" style="margin-bottom:1.5rem;">
      <div class="card-header">
        <div class="card-title">
          <i class="fa-solid fa-sliders" style="color:var(--accent-cyan);"></i> Report Parameters & Custom Investigator Notes
        </div>
      </div>
      ...
      <div style="display:grid; grid-template-columns: 2fr 1fr; gap:1.5rem;">
        <div>
          ...
          <input type="text" id="report-title-input" value="Digital Forensics Examination & Cyber Triage Report - ${activeCase.case_code}" style="width:100%; padding:0.6rem; background:var(--bg-tertiary); border:1px solid var(--border-color); border-radius:6px; color:#FFF; font-family:inherit; margin-bottom:1rem;" />

          <label style="display:block; font-size:0.8rem; font-weight:600; color:var(--text-secondary); margin-bottom:0.35rem;">Investigator Concluding Notes & Recommendations</label>
        ...
        <div style="background:var(--bg-secondary); padding:1rem; border-radius:8px; border:1px solid var(--border-color); font-size:0.825rem;">
          <div style="font-weight:700; color:var(--accent-cyan); margin-bottom:0.75rem; text-transform:uppercase; font-size:0.75rem;">
            Sections Included in Final PDF
          </div>
          <div style="display:flex; flex-direction:column; gap:0.4rem; color:var(--text-secondary);">
            ...
            <div><i class="fa-solid fa-check" style="color:var(--sev-low);"></i> 2. Case Metadata & Lead Examiner</div>
            ...
            <div><i class="fa-solid fa-check" style="color:var(--sev-low);"></i> 7. AI Disclaimer & Forensic Limitations</div>
          </div>
        </div>
      </div>
    </div>
      ...
      previewSheet.innerHTML = `
        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.8rem; margin-bottom:1.5rem; background:#F8FAFC; border:1px solid #E2E8F0;">
          <tr>
            ...
            <td style="padding:0.5rem 0.75rem; border:1px solid #E2E8F0;">${activeCase.name}</td>
          ...
          <tr>
            ...
            <td style="padding:0.5rem 0.75rem; border:1px solid #E2E8F0; font-weight:700;">Lead Examiner:</td>
            <td style="padding:0.5rem 0.75rem; border:1px solid #E2E8F0;">${activeCase.investigator}</td>
          </tr>
        </table>
        ...
        <p style="font-size:0.85rem; line-height:1.6; color:#334155; margin-bottom:1.25rem;">
          A digital forensic examination was executed for incident <strong>${activeCase.case_code}</strong>. 
        ...
        <!-- AI Disclaimer & Integrity Notes -->
        <div style="border-top:1px solid #CBD5E1; padding-top:1rem; font-size:0.75rem; color:#64748B; line-height:1.5;">
          <p><strong>Examiner Certification & AI Disclaimer:</strong> This digital forensic report incorporates correlation telemetry analyzed with CYBERTRIAGE AI. All assertions maintain full traceability back to raw evidence hashes. Evidence files are preserved in read-only forensic storage.</p>
        </div>
      `;
    } catch (err) {
  ...
  generatePdfBtn.onclick = async () => {
    ...
    try {
      ...
      generatePdfBtn.innerHTML = `<i class="fa-solid fa-check"></i> PDF Downloaded!`;
      setTimeout(() => {
